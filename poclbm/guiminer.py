...
class VertListenerThread(MinerListenerThread):
    LINES = [
        (r" Accepted .*",
            lambda _: UpdateAcceptedEvent(accepted=True)),
        #(r"A:*\d+",
...
class MinerTab(wx.Panel):
    ...
    """
    def __init__(self, parent, id, devices, servers, defaults, gpusettings_data, gpusettingsvert_data, statusbar, data):
        wx.Panel.__init__(self, parent, id)
        self.parent = parent
        self.servers = servers
        self.defaults = defaults
        self.gpusettings_data = gpusettings_data
        self.gpusettingsvert_data = gpusettingsvert_data
        ...
        self.gpusettings = wx.ComboBox(self, -1,
                                  ...
                                  style=wx.CB_READONLY)
        self.gpusettingsvert_lbl = wx.StaticText(self, -1, _("GPU Defaults:"))
        self.gpusettingsvert = wx.ComboBox(self, -1,
                                  choices=[s['name'] for s in gpusettingsvert_data],
                                  style=wx.CB_READONLY)
        ...
        self.minervertminer_lbl = wx.StaticText(self, -1, _("Miner: vertminer"))
        ...
        self.labels = [self.minercgminer_lbl, self.minerreaper_lbl, 
                      ...
                      self.minercudaminer_lbl, self.minervertminer_lbl]
        ...
        self.gpusettingsvert.Bind(wx.EVT_COMBOBOX, self.on_select_gpusettingsvert)
    ...
    @property
    def gpusettingsvert_config(self):
        profilename = self.gpusettingsvert_data.GetValue()
        return self.get_gpusettingsvert_by_field(profilename, 'name')
    ...
    def configure_subprocess_vertminer(self):
        """Set up the command line for cgminer."""

        # Set the path for cgminer, should be ./vertminer/vertminer.exe
        # Not set up for unix, modify this to /vertminer/vertminer for unix
        os.chdir(STARTUP_PATH)
        path = '\"' + STARTUP_PATH + "\\vertminer\\vertminer.exe" + '\"'
        vertdir = STARTUP_PATH + "\\vertminer\\"
        ...
        # -T <disable curses interface and output to console (stdout)>
        # -g <GPU threads>
        cmd = "%s --scrypt-vert -u %s -p %s -o %s%s:%s --gpu-platform %s -d %s -w %s -v %s -I %s -g %s -l 1 -T %s --thread-concurrency %s" % (
            path,
            self.txt_username.GetValue(),
            self.txt_pass.GetValue(),
            http_header,
        ...
        # Full console command for batch file creation given below; don't add -T in this instance so end user gets full output
        full_console_cmd = "%s --scrypt-vert -u %s -p %s -o %s%s:%s --gpu-platform %s -d %s -w %s -v %s -I %s -g %s -l 1 %s --thread-concurrency %s" % (
            path,
            self.txt_username.GetValue(),
            self.txt_pass.GetValue(),
            http_header,
        ...
        f = open(vertdir + "mine-" +  self.name + ".bat", 'w')
        f.write(full_console_cmd)
        f.close()
    ...
    def configure_subprocess_cudaminer(self):
        ...
        # -o host and port prefixed with http_header
        # -O username:password
        ...
        cmd = "%s -o %s%s:%s/ -O %s:%s -d %s -i %s -l %s -C %s -m %s" % (
            path,
            http_header,
            self.host_without_http_prefix,
            self.txt_port.GetValue(),
    ...
    def start_mining(self):
        ...
        elif "VERTMINER" in self.external_path:
            conf_func = self.configure_subprocess_vertminer
            listener_cls = VertListenerThread
        ...
        try:
            ...
            elif conf_func == self.configure_subprocess_vertminer:
                vertminer_env = os.environ # Create an environment to set below environmental variable in
                vertminer_env['GPU_MAX_ALLOC_PERCENT'] = "400" # Set this environmental variable so we can use high thread concurrencies in vertminer
                vertminer_env[' GPU_USE_SYNC_OBJECTS'] = "1" # No idea what this does, vertminer
                self.miner = subprocess.Popen(cmd,
                                              env=vertminer_env,
                                              stdout=subprocess.PIPE,
                                              stderr=None,
    ...
    def on_select_gpusettingsvert(self, event):
        """Update our info in response to a new server choice."""
        new_gpusettingsvert_name = str(self.gpusettingsvert.GetValue())
        new_gpusettingsvert = self.get_gpusettingsvert_by_field(new_gpusettingsvert_name, 'name')
        self.change_gpusettingsvert(new_gpusettingsvert)
    ...
    def get_gpusettingsvert_by_field(self, target_val, field):
        """Return the first server dict with the specified val, or {}."""
        for s in self.gpusettingsvert_data:
            if s.get(field) == target_val:
                return s
        return {}           
    ...
    def set_tooltips(self):
        ...
        add_tooltip(self.gpusettingsvert, _("Default values for any given AMD video card.\nTry these first if you are new to VertCoin mining."))
    ...
    def change_gpusettingsvert(self, new_gpusettingsvert):
        self.reset_statistics()
        if 'thread_concurrency' in new_gpusettingsvert:
            self.txt_thrcon.SetValue(str(new_gpusettingsvert['thread_concurrency']))
        if 'worksize' in new_gpusettingsvert:
            self.txt_worksize.SetValue(str(new_gpusettingsvert['worksize']))
        if 'vectors' in new_gpusettingsvert:
            self.txt_vectors.SetValue(str(new_gpusettingsvert['vectors']))
        if 'gputhreads' in new_gpusettingsvert:
            self.txt_gputhreads.SetValue(str(new_gpusettingsvert['gputhreads']))
        if 'intensity' in new_gpusettingsvert:
            self.txt_intensity.SetValue(str(new_gpusettingsvert['intensity']))
    ...
    def layout_minertype(self, row):
        ...
        elif self.external_path == "VERTMINER":
            self.inner_sizer.Add(self.minervertminer_lbl, (row, 0), flag=LBL_STYLE)
        else:
    ...
    def layout_gputhreads_gpusettingsvert(self, row):
        """
        Like it sounds, no. gpu threads and gpu defaults
        """
        self.inner_sizer.Add(self.gputhreads_lbl, (row, 0), flag=LBL_STYLE)
        self.inner_sizer.Add(self.txt_gputhreads, (row, 1), flag=wx.EXPAND)
        self.inner_sizer.Add(self.gpusettingsvert_lbl, (row, 2), flag=LBL_STYLE)
        self.inner_sizer.Add(self.gpusettingsvert, (row, 3), flag=wx.EXPAND)
    ...
    def layout_default(self):
        ...
        if self.external_path == "VERTMINER":
            self.layout_gputhreads_gpusettingsvert(row=row + 6 + int(is_custom))
        else:
        ...
        if self.external_path == "CGMINER":
            self.set_widgets_visible([self.minerreaper_lbl, self.proxy_lbl, self.gpusettingsvert_lbl, self.gpusettingsvert, self.minervertminer_lbl], False)
        elif self.external_path == "VERTMINER":
            self.set_widgets_visible([self.minerreaper_lbl, self.proxy_lbl, self.gpusettings_lbl, self.gpusettings, self.minercgminer_lbl, self.minerreaper_lbl, self.proxy_lbl], False)
        elif self.external_path == "REAPER":
            self.set_widgets_visible([self.minercgminer_lbl, self.proxy_lbl, self.stratum_lbl, self.txt_stratum, self.flags_lbl, self.txt_flags, self.gpusettingsvert_lbl, self.gpusettingsvert, self.minervertminer_lbl], False)
        else:
            self.set_widgets_visible([self.minercgminer_lbl, self.minerreaper_lbl], False)
            
        if self.external_path == "PROXY":
            self.set_widgets_visible([self.user_lbl, self.pass_lbl, self.device_lbl, self.thrcon_lbl, self.worksize_lbl, self.vectors_lbl, self.intensity_lbl, self.gputhreads_lbl, self.gpusettings_lbl, self.gpusettingsvert_lbl,  self.stratum_lbl, self.minervertminer_lbl], False)
            self.set_widgets_visible([self.txt_username, self.txt_pass, self.device_listbox, self.txt_thrcon, self.txt_worksize, self.txt_vectors, self.txt_intensity, self.txt_gputhreads, self.gpusettings, self.gpusettingsvert, self.txt_stratum], False) 
        else:
    ...
    def layout_cudaminer(self):
        ...
        self.set_widgets_visible([self.balance_lbl,
                                  ...
                                  self.gpusettingsvert,
                                  self.gpusettingsvert_lbl,
                                  self.stratuminfo0_lbl,
                                  self.stratuminfo1_lbl,
                                  self.proxy_lbl,
                                  self.minervertminer_lbl,
...
class GUIMiner(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        gpusettingsvert_config_path = os.path.join(get_module_path(), 'gpusettings_vert.ini')
        with open(gpusettingsvert_config_path) as f:
            data = json.load(f)
            self.gpusettingsvert_data = data.get('gpusettingsvert')
        ...
        ID_NEW_EXTERNAL, ID_NEW_PHOENIX, ID_NEW_CGMINER, ID_NEW_VERTMINER, ID_NEW_REAPER, ID_NEW_CUDAMINER, ID_NEW_PROXY, ID_NEW_UFASOFT = wx.NewId(), wx.NewId(), wx.NewId(), wx.NewId(), wx.NewId(), wx.NewId(), wx.NewId(), wx.NewId()
        ...
        new_menu.Append(ID_NEW_CGMINER, _("New cgminer..."), _("Create a new cgMiner"), wx.ITEM_NORMAL)
        new_menu.Append(ID_NEW_VERTMINER, _("New vertminer..."), _("Create a new vertminer"), wx.ITEM_NORMAL)
        ...
        self.Bind(wx.EVT_MENU, self.new_vertminer_profile, id=ID_NEW_VERTMINER)
    ...
    def add_profile(self, data={}):
        ...
        panel = MinerTab(self.nb, -1, self.devices, self.servers,
                             self.defaults, self.gpusettings_data, self.gpusettingsvert_data, self.statusbar, data)
    ...
    def new_vertminer_profile(self, event):
        """Create a new miner using the vertminer OpenCL miner backend."""
        path = os.path.join(get_module_path(), 'vertminer.exe')
        self.name_new_profile(extra_profile_data=dict(external_path="VERTMINER"))
